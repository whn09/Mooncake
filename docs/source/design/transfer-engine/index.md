...
- `MC_MAX_CONCURRENT_REG_MR` Cap on how many buffers `registerLocalMemoryBatch` registers concurrently (EFA transport). The default 0 selects the built-in value of 8. The batch previously spawned one thread per buffer with no cap, which is counterproductive for models that register hundreds of buffers at once — registration pins pages and serializes on the provider's per-domain lock, so surplus threads add contention rather than throughput. Measured on a 192-core p6-b300 node registering 138 GPU buffers across 16 NICs, varying only this cap: unbounded 138.7 s, 128 threads 130.1 s, 32 threads 51.0 s, 8 threads 20.7 s, 4 threads 24.8 s. Note the default is not core-scaled, since the bottleneck is the provider lock rather than CPU.
